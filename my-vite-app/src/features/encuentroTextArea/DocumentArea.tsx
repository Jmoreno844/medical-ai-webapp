  ...
  const handleContentUpdate = useCallback(
    async (documentId: number, content: string) => {
      ...
      try {
        ...
        console.error("[DOC_UPDATE] Error saving streamed content:", err);
      }
    },
    ...
    if (
      ...
      const checkContentLength = async () => {
        if (documentContentCache && transcriptionDocId) {
          const cachedContent = documentContentCache.get(transcriptionDocId);
        ...
        if (fetchDocumentContent && transcriptionDocId) {
          try {
            const content = await fetchDocumentContent(transcriptionDocId);
            ...
            console.log("[DOC_AREA] Error checking current content:", err);
          }
        }
      ...
      const attemptContentRefresh = async (
        ...
        if (hasContent) {
          console.log(`[DOC_AREA] Content already available, stopping polling`);
          return;
        }
        ...
        if (attempt < maxAttempts) {
          ...
          const nextDelay = Math.min(500 * Math.pow(1.5, attempt - 1), 1000);
