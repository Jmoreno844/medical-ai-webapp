...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--document-file",
        default=None,
        help="Fixture JSON under cases/context (defaults to first in case).",
    )
...
def main() -> int:
    ...
    try:
        result, llm_responses = run_extract_fixture(
            fixture=fixture,
            cases_dir=cases_dir,
    ...
    if output_detail == "full":
        output_payload["raw_responses"] = [
            response.content for response in llm_responses
        ]
