...
from context_pipeline.decompose.decompose import run_decompose  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Debug doctor-note decompose into clinical claims."
    )
...
def main() -> int:
    ...
    output_path = results_dir / (
        f"{run_started_at.strftime('%Y%m%dT%H%M%SZ')}_debug_{case_meta.id}_{provider}.json"
    )

    print(
        f"case={case_meta.id} session={case_meta.session_id} "
        f"provider={provider} model={model} prompt_version={prompt_version}"
    )
    try:
        claims, llm_response = run_decompose(
    ...
    output_payload: dict[str, object] = {
        "provider": provider,
        "model": model,
        "decompose_result": enrich_decompose_result_for_export(claims),
    }
    ...
    print("\n" + format_decompose_debug_output(claims))
    print(f"\nWrote {output_path}")
    return 0
