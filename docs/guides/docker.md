...
| `docker-compose.yml` | **Local dev**: `web` + `db`; `web` uses **develop** settings and `runserver` on 8000. DB init mounts `scripts/docker/init-db.sh`. |
...
- Default: `docker compose up web db` — API on http://localhost:8000.
