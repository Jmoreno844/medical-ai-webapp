...
El copiloto ya tiene un boundary real `frontend -> FastAPI broker -> copilot-agent-service`,
...
En el sidechat actual, `thread_id` ya identifica una conversación real: FastAPI crea uno nuevo al iniciar chat, el frontend lo mantiene solo en memoria del panel y LangGraph lo usa como checkpoint key para conservar contexto entre mensajes del mismo chat.

## Deudas activas

### 1. Shared JWT temporal entre FastAPI y `copilot-agent-service`

- **Impacto:** `local` y `stg` usan un secreto compartido (`COPILOT_SERVICE_SHARED_JWT`) tanto para el broker `FastAPI -> copilot-agent-service` como para las tools internas `copilot_agent -> FastAPI`, en vez de auth service-to-service con OIDC/ID token.
...
- **Owner:** `backend_fastapi/` + `copilot_agent/` + `infra/`.
...
- **Impacto:** FastAPI llama al agent runtime por HTTP directo para crear runs y leer eventos.
...
- **Owner:** `backend_fastapi/app/domains/copilot/`.
...
- **Owner:** `backend_fastapi/app/domains/copilot/` + `backend_fastapi/app/domains/documents/` + `webapp/`.
...
- **Owner:** `backend_fastapi/` + arquitectura de producto.
...
- **Owner:** `copilot_agent/` + `backend_fastapi/app/domains/copilot/` + diseño de producto del QA helper.
...
- **Owner:** `copilot_agent/` + `backend_fastapi/app/domains/copilot/`.
...
- **Qué requeriría la solución completa:**
  ...
  - Abrir un canal SSE desde el copilot agent hacia FastAPI broker (análogo al SSE de document generation pero brokered).
  - FastAPI reemitiría los eventos parciales al browser vía el SSE existente o uno nuevo.
...
- **Owner:** `copilot_agent/app/runtime.py` + `backend_fastapi/app/domains/copilot/` + `webapp/`.
...
- **Owner:** `backend_fastapi/app/domains/documents/` + `backend_fastapi/app/domains/copilot/` + `copilot_agent/`.
