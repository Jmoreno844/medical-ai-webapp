...
async def test_no_speech_section_skips_gemini(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_vad(_object_name: str) -> VadResult:
    ...
    monkeypatch.setattr("app.processor.transcribe_gcs_audio", fail_gemini)
...
async def test_vad_error_fails_open_to_gemini(
    ...
    monkeypatch.setattr(processor, "_run_vad", fake_vad)
    monkeypatch.setattr("app.processor.transcribe_gcs_audio", fake_gemini)
