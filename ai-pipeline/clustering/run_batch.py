...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run conversation clustering across cases and models."
    )
            ...
            for model_spec in model_specs:
                ...
                try:
                    ...
                    output_entry["clustering_result"] = (
                        enrich_clustering_result_for_export(
                            clustering_result, catalog
                        )
                    )
                    output_entry["turn_coverage"] = coverage.to_dict()
                    output_entry["repair_passes"] = [
                        repair_pass.to_dict()
                    ...
                    if not coverage.is_complete:
                        ...
                        print(
                            "WARNING: incomplete turn coverage "
                            f"case={case.id} model={model_label} "
                            f"missing={coverage.missing_turn_ids} "
                            f"extra={coverage.extra_turn_ids} "
                            f"duplicates={coverage.duplicate_turn_ids}"
                        )
                except Exception as exc:
                    output_entry["error"] = str(exc)
                case_entry["outputs"].append(
                    format_clustering_output_for_detail(output_entry, output_detail)
                )
                _persist_results(output_path, results)
