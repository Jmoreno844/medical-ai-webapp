...
from apps.documents.services.rich_document_content import tiptap_json_to_markdown

_MARKDOWN_HEADING_RE = re.compile(r"^\s{0,3}(#{1,6})\s+(?P<heading>.+?)\s*$")
_BOLD_HEADING_RE = re.compile(
    r"^\s*\*\*(?P<heading>[^*\n]{2,120}?)\*\*:?\s*(?P<rest>.*)$"
)
_ALL_CAPS_HEADING_RE = re.compile(r"^\s*(?P<heading>[A-ZÁÉÍÓÚÑ0-9][A-ZÁÉÍÓÚÑ0-9 /()\-.]{2,100})\s*$")
...
def _normalize_heading(value: str | None) -> str:
    ...
    if not text:
        return ""
    text = _HEADING_NUMBER_PREFIX_RE.sub("", text)
    text = text.rstrip(":.- ")
...
def extract_document_sections(
    *,
    ...
    for index, section in enumerate(sections):
        ...
        section_content = markdown[section["start_offset"]:next_start].strip()
        section["content_preview"] = _content_preview(section_content)
