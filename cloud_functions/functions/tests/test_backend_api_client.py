...
def test_backend_api_base_url_targets_fastapi_v1(monkeypatch):
    ...
    assert django_api.get_api_base_url() == "https://backend.example.com/api/v1"
...
def test_backend_api_base_url_uses_configured_api_version(monkeypatch):
    ...
    assert django_api.get_api_base_url() == "https://backend.example.com/api/v2"
...
def test_backend_api_base_url_strips_any_existing_version(monkeypatch):
    ...
    assert django_api.get_api_base_url() == "https://backend.example.com/api/v3"


def test_backend_api_base_url_falls_back_to_legacy_env(monkeypatch):
    monkeypatch.delenv("BACKEND_API_BASE_URL", raising=False)
    monkeypatch.setenv("DJANGO_API_BASE_URL", "https://backend.example.com")

    assert django_api.get_api_base_url() == "https://backend.example.com/api/v1"


@patch("services.django_api.requests.patch")
@patch("services.django_api.notify_transcription_complete")
def test_update_document_content_calls_fastapi_v1(
    ...
    result = django_api.update_document_content(123, "hola", "token")
...
@patch("services.django_api.requests.post")
def test_send_generation_chunk_calls_fastapi_v1(mock_post, monkeypatch):
    ...
    result = django_api.send_generation_chunk(
        document_id=123,
        process_id="gen_123",
        chunk="hola",
        token_auth="token",
