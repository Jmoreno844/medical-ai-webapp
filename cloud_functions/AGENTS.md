...
- `functions/services/django_api.py` — callbacks to Django
...
- `services/django_api.py` is the only place that should know Django callback URLs.
- Cloud Functions do not own persistence; Django remains the system of record.
...
- Keep callback JWT handling compatible with Django docs and backend code.
...
- Preserve `validate_only` behavior in document generation; Django depends on it before spawning the background kickoff.
...
- If you change request/response payloads, verify the matching Django callback endpoint in `backend/apps/documents/api/`
