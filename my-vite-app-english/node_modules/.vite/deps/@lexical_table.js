...
function $insertTableColumn__EXPERIMENTAL(insertAfter = true) {
  ...
  rowLoop: for (let i = 0; i < rowCount; i++) {
    ...
    if (insertAfterColumn < 0) {
      $insertFirst2(loopRow, $createTableCellNodeForInsertTableColumn(headerState));
      continue;
    }
    const {
      cell: currentCell,
...
function $insertFirst2(parent, node) {
    ...
    for (let i = 1; i < rowSpan; i++) {
      ...
      if (insertAfterCell === null) {
        for (let j = colSpan - 1; j >= 0; j--) {
          $insertFirst2(currentRowNode, $createTableCellNode(colStyles[j] | rowStyles[i]).append($createParagraphNode()));
        }
      } else {
        for (let j = colSpan - 1; j >= 0; j--) {
...
var ARROW_KEY_COMMANDS_WITH_DIRECTION = [[KEY_ARROW_DOWN_COMMAND, "down"], [KEY_ARROW_UP_COMMAND, "up"], [KEY_ARROW_LEFT_COMMAND, "backward"], [KEY_ARROW_RIGHT_COMMAND, "forward"]];
var DELETE_TEXT_COMMANDS = [DELETE_WORD_COMMAND, DELETE_LINE_COMMAND, DELETE_CHARACTER_COMMAND];
var DELETE_KEY_COMMANDS = [KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND];
...
var TableNode = class _TableNode extends ElementNode {
  ...
  exportDOM(editor) {
    ...
    return {
      ...
      element: !isHTMLTableElement(element) && isHTMLElement(element) ? element.querySelector("table") : element
    };
  }
  canBeEmpty() {
    return false;
